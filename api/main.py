...
# Try initial load, but don't crash if it fails
try:
    if os.path.exists(MODEL_PATH):
        current_model = joblib.load(MODEL_PATH)
        print(f"Initial model loaded from {MODEL_PATH}")
    else:
        print("No local model found. API will start and wait for ClearML sync.")
...
def load_latest_model():
    ...
    try:
        # Find the latest successful training task in the project
        ...
        if last_task and last_task.id != current_model_id:
            ...
            if models and 'output' in models:
                # downloads\ing the model to local cache
...
async def startup_event():
    # Trigger an immediate check so the API gets the model ASAP on boot
    asyncio.create_task(model_check_loop())

@app.post("/predict")
async def predict(data: list):
    if current_model is None:
        return {"error": "Model is not yet loaded. Please wait about 30 seconds for ClearML sync."}
    
    # data is expected to be the sequence of prices
